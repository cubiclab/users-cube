...
class AccessController extends Controller
{
    ...
    public function actionUpdateRole($name)
    {
        ...
        if ($role instanceof Role) {
            ...
            throw new BadRequestHttpException(Yii::t('userscube', 'Страница не найдена'));
        }
    }
    ...
    public function actionUpdatePermission($name)
    {
        ...
        } else throw new BadRequestHttpException(Yii::t('userscube','Страница не найдена'));
    }
    ...
    protected function validate($field, $regex)
    {
        ...
        if ($validator->validate($field, $error))
            ...
            $this->error[] = Yii::t('userscube','Значение "{field}" содержит не допустимые символы', ['field' => $field]);
            return false;
        }
    }
    ...
    protected function isUnique($name, $type)
    {
        if ($type == 'role') {
            $role = Yii::$app->authManager->getRole($name);
            if ($role instanceof Role) {
                $this->error[] = Yii::t('userscube','Роль с таким именем уже существует: ') . $name;
                return false;
            } else return true;
        } elseif ($type == 'permission') {
            $permission = Yii::$app->authManager->getPermission($name);
            if ($permission instanceof Permission) {
                $this->error[] = Yii::t('userscube','Правило с таким именем уже существует: ') . $name;
                return false;
            } else return true;
        }
    }
